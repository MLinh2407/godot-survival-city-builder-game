...
# Setter fires the signal and clamps automatically.
var hope_order_slider: float:
	set(value):
		hope_order_slider = clampf(value, 0.0, 100.0)
		slider_changed.emit(hope_order_slider)
...
# Called by CrisisEventSystem when a choice is made.
# Negative amount = shift toward Hope (0)
# Positive amount = shift toward Order (100)
func shift_slider(amount: float) -> void:
	hope_order_slider += amount
	print("GameManager: Slider shifted by %.1f → now %.1f" % [amount, hope_order_slider])

func get_slider_zone() -> String:
	if hope_order_slider <= GameConstants.SLIDER_HOPE_UPPER:
		return "Hope"
	elif hope_order_slider >= GameConstants.SLIDER_ORDER_LOWER:
		return "Order"
	return "Neutral"